...
func SortAlgorithm(s *stack.Stack) {
	...
	for !IsSorted(s.StackA) || len(s.StackB) != 0 {
		if s.StackA[0] == Max(s.StackA) {
			...
			continue
		} else if Min(s.StackA) == s.StackA[len(s.StackA)-1] {
			...
			for len(s.StackB) > 0 {
				...
				if len(s.StackA) > 1 && len(s.StackB) > 1 && s.StackA[0] > s.StackA[1] && s.StackA[0] != Max(s.StackA) && s.StackB[0] < s.StackB[1] {
					...
					continue
				} else if len(s.StackA) > 0 && len(s.StackB) > 0 && s.StackA[0] == Max(s.StackA) && s.StackB[0] == Min(s.StackB) {
					instructions.ExecuteInstruction(s, "rr")
					continue
				} else if len(s.StackA) > 0 && s.StackA[0] == Max(s.StackA) {
					instructions.ExecuteInstruction(s, "ra")
					continue
				} else if len(s.StackB) > 0 && s.StackB[0] == Min(s.StackB) {
					instructions.ExecuteInstruction(s, "rb")
					continue
				} else if len(s.StackA) > 0 && len(s.StackB) > 0 && s.StackA[len(s.StackA)-1] == Min(s.StackA) && s.StackB[len(s.StackB)-1] == Max(s.StackB) {
					instructions.ExecuteInstruction(s, "rrr")
					continue
				} else if len(s.StackA) > 0 && s.StackA[len(s.StackA)-1] == Min(s.StackA) {
					instructions.ExecuteInstruction(s, "rra")
					continue
				} else if len(s.StackB) > 0 && s.StackB[len(s.StackB)-1] == Max(s.StackB) {
